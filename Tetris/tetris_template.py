...
#        if shape.can_move():
        ...
        for block in self.get_blocks():
            new_x = self.center_block.x - direction*self.center_block.y + direction*block.y
            new_y = self.center_block.y + direction*self.center_block.x - direction*block.x
...
class Board():
    ...
    def move_down_rows(self, y_start):
        ...
        # Move from y_start up to 0
        for y in range(y_start,0-1,-1):
            for x in range(0,self.width):
                coords = (x, y)
    ...
    def game_over(self):
        ...
        #YOUR CODE HERE
        #pass
#        msg1 = Text(Point(self.width/2-50,self.height/2-25), "Game Over!")
        msg1 = Text(Point(150,200), "Game Over!")
        msg1.setSize(24)
...
class Tetris():
    ...
    def create_new_shape(self):
        ...
        # Get random object in array
        # Create the object with the coordinates
        # Return it
        index = random.randint(0,len(self.SHAPES)-1)
        shape = self.SHAPES[index]
        
        new_shape = shape(Point(self.BOARD_WIDTH/2,0))
    ...
    def do_move(self, direction):
        ...
        distance = Point(self.DIRECTION[direction][0], self.DIRECTION[direction][1])
        dx = distance.getX()
        dy = distance.getY()
        ...
        if self.current_shape.can_move(self.board, dx, dy):
            ...
            # Add a new piece if the move was "Down"
            if direction == "Down":
                if (self.board.can_move(self.BOARD_WIDTH/2,0)):
