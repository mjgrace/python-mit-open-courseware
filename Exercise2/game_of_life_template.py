...
class Board(object):
    ...
    def __init__(self, win, width, height):
        ...
        for x in range(0,self.width):
            for y in range(0,self.height):
                self.block_list[str(x)+"-"+str(y)] = Block(Point(x,y), 'blue')
    ...
    def seed(self, block_coords):
        ...
        for coord in block_coords:
            ...
            block = self.block_list[str(coord[0])+"-"+str(coord[1])]
            block.set_live(self.canvas)
    ...
    def get_block_neighbors(self, block):
        ...
        for x in range(max(block_x - 1, 0), (min(block_x + 2, self.width))):
            for y in range(max(block_y - 1, 0), (min(block_y + 2, self.height))):
                if (x != block_x or y != block_y):
                    neighbors.append(self.block_list[str(x)+"-"+str(y)])
    ...
    def simulate(self):
        ...
        for block in board.block_list.values():
            ...
            alive_blocks = 0
            for neighbor in neighbors:
                # TODO - Why isn't is_live working?
                print neighbor.get_coords(), neighbor.is_live()
                if neighbor.is_live():
                    alive_blocks = alive_blocks + 1
            if ((block.is_live() and (alive_blocks == 2 or alive_blocks == 3))
                or ((not(block.is_live())) and alive_blocks == 3)):
                block.set_live(self.canvas)
            else:
                block.set_dead()
